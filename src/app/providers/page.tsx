...
export default function ProvidersPage() {
  const allProviders = useMemo(() => getProviders(), []);
  ...
  const filtered = useMemo(() => {
    ...
    switch (sortBy) {
      case "price-asc":
        result.sort((a, b) => Math.min(...a.models.map(m => m.inputPrice)) - Math.min(...b.models.map(m => m.inputPrice)));
        break;
      case "price-desc":
        result.sort((a, b) => Math.min(...b.models.map(m => m.inputPrice)) - Math.min(...a.models.map(m => m.inputPrice)));
        break;
      case "newest":
        result.sort((a, b) => new Date(b.firstSeen).getTime() - new Date(a.firstSeen).getTime());
        break;
      case "response":
        result.sort((a, b) => (a.responseTime || 9999) - (b.responseTime || 9999));
        break;
      default:
    ...
    return result;
  }, [allProviders, search, billingType, status, sortBy]);
