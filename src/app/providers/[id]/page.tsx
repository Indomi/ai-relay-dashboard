  ...
  const similarProviders = allProviders
    .filter(
      (p) =>
        p.id !== provider.id &&
        p.models.some((m) =>
          provider.models.some((pm) => pm.model === m.model)
        )
    )
    .slice(0, 4);
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 text-sm text-muted-foreground">
                ...
                <span className="flex items-center gap-1">
                  ...
                  更新于 {formatRelativeTime(provider.lastUpdated)}
                </span>
              </div>
                  ...
                  <tbody>
                    {provider.models.map((model) => (
                      <tr key={model.model} className="border-b last:border-0">
                        ...
                        <td className="py-2.5 text-right text-purple-600 font-medium">
            ...
            <Card className="border-0 shadow-sm">
              <CardContent className="p-5">
                ...
                <p className="text-xl font-bold text-emerald-600">
                  ${provider.freeQuota.amount}
                </p>
                <p className="text-xs text-muted-foreground">
                  {provider.freeQuota.unit}
                </p>
              </CardContent>
            </Card>
          )}
                ...
                {provider.sources.map((source, i) => (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <Badge
                        variant="secondary"
                        className={`text-xs ${getPlatformColor(source.platform)}`}
                      >
                    ...
                    <p className="text-sm truncate group-hover:text-blue-600 transition-colors">
                      {source.title}
                    </p>
